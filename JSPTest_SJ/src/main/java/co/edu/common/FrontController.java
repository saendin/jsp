...
public class FrontController extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		/*
		 *참고 : .do로 끝나는 타입은 co.edu.common.FrontController를 실행하게끔 xml 서버매핑
		 *참고 : 각 controller클래스들 안에서는 .tiles타입을 실행시키기로 xml 서버매핑
		*/
		...
		mappings.put("/addBoard.do", new AddBoardControl()); // 게시글 등록화면 --> AddBoardControl()로 보냄
		mappings.put("/insertBoard.do", new InsertBoardControl());//게시글 등록처리
		mappings.put("/boardList.do", new BoardListControl()); // 게시글 목록
		mappings.put("/boardListPaging.do", new BoardListPagingControl()); // 페이징처리 컨트롤
	}
